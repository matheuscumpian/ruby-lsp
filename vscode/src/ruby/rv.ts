...
export class Rv extends VersionManager {
  async activate(): Promise<ActivationResult> {
    ...
    const version = await this.findVersion(rvExec);

    const parsedResult = await this.runEnvActivationScript(`${rvExec} ruby run ${version} --`);
  ...
  private async findVersion(rvExec: string): Promise<string> {
    try {
      const result = await this.runScript(`${rvExec} ruby pin`);
      // Output format: "/path/to/.ruby-version is pinned to Ruby ruby-3.4.8"
      const match = /pinned to Ruby (.*)$/.exec(result.stdout.trim());
      
      if (match && match[1]) {
        return match[1];
      }
    } catch (error: any) {
      // If rv ruby pin fails, it usually means no version is pinned
      this.outputChannel.error(`Failed to find pinned Ruby version with rv: ${error.message}`);
    }

    throw new Error("Could not find a pinned Ruby version using rv. Please ensure a .ruby-version file exists.");
  }
}
